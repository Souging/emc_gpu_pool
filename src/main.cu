...
__constant__ uint8_t CURRENT_CHALLENGE[32] = {0x78, 0x92, 0x55, 0x68, 0x00, 0x99, 0x00, 0x90,
...
__global__ void calculate() {
    ...
    __shared__ uint8_t shared_difficulty[4];
    __shared__ uint8_t shared_challenge[32];
    __shared__ uint8_t shared_miner_address[20];
    if (threadIdx.x < 4) shared_difficulty[threadIdx.x] = DIFFICULTY[threadIdx.x];
...
int main(int argc, char *argv[]) {
    ...
    cudaMemcpyToSymbol(DIFFICULTY, "\x22\x33\x99\x99", 4);
    cudaMemcpyToSymbol(CURRENT_CHALLENGE, "\x78\x92\x55\x68\x00\x99\x00\x90\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 32);
